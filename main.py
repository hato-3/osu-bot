...
class osutop_modal(ui.Modal, title='Enter Page Number'):
    ...
    async def on_submit(self, inter: Interaction):
        cur.execute(
            "SELECT user_id, i, max FROM osutop_temp WHERE message_id = ?",
            (inter.message.id, ))
        ...
        embed = await _osu_top(user_id, i)
...
class recent_modal(ui.Modal, title='Enter Page Number'):
    ...
    async def on_submit(self, inter: discord.Interaction):
        cur.execute(
            "SELECT username, user_id, i, max FROM rs_temp WHERE message_id = ?",
            (inter.message.id, ))
        ...
        embed = await _recent(user_id, i)
        await inter.followup.edit_message(embed=embed,
                                          message_id=inter.message.id)
...
async def osuset(ctx, *, arg):
    ...
    cur = conn.cursor()
    cur.execute(
        'INSERT INTO user(discord_user_id, osu_username, osu_user_id) values(?, ?, ?) ON CONFLICT(discord_user_id) DO UPDATE SET osu_username = ?, osu_user_id = ?',
        (ctx.author.id, username, user_id, username, user_id),
    )
    conn.commit()

    embed = await _osu_profile(ctx, username)
...
async def osu(ctx, *arg):
    ...
    if not arg:
        ...
        cur = conn.cursor()
        id = ctx.author.id
        cur.execute('SELECT osu_user_id FROM user WHERE discord_user_id = ?',
                    (id, ))
        ...
        cur.execute(
            'UPDATE user SET osu_username = ? WHERE discord_user_id = ?', (
                username,
                id,
            ))
        conn.commit()
...
async def _osu_profile(ctx, username):
    ...
    try:
        ...
        datetime_rank_highest = datetime.datetime.timestamp(
            datetime_rank_highest)
    except:
    ...
    if type(globalrank) != int:
        if rank_highest_at == None:
            embed = discord.Embed(
                ...
                .format(level, levelplus, pp, acc, playcount, playtime,
                        rank_SSH, rank_SS, rank_SH, rank_S, rank_A),
                color=discord.Colour.green())
        else:
            embed = discord.Embed(
                description=
                "▸ **Global Ranking**: #-\n▸ **Country Ranking**: #-\n▸ **Peak Rank**: #{:,d} achieved <t:{}:R>\n▸ **Level**: {:d} + {:.2f}%\n▸ **PP**: {:,.2f} **ACC**: {:.2f}%\n▸ **Playcount**: {:,d} ({:,.0f}hrs)\n▸ **Ranks**: **SSH** {:d} **SS** {:d} **SH** {:d} **S** {:d} **A** {:d}"
                .format(rank_highest, int(datetime_rank_highest), level,
                        levelplus, pp, acc, playcount, playtime, rank_SSH,
        ...
        embed = discord.Embed(
            ...
            .format(globalrank, countryrank, rank_highest,
                    int(datetime_rank_highest), level, levelplus, pp, acc,
                    playcount, playtime, rank_SSH, rank_SS, rank_SH, rank_S,
                    rank_A),
            color=discord.Colour.green())
    embed.set_thumbnail(url=avatar)
    embed.set_author(name="osu! Standard Profile for {}".format(game_id),
                     url="https://osu.ppy.sh/users/{}".format(user_id),
                     icon_url=flag_url)
...
async def rs(ctx, *arg):
    ...
    if not arg:
        ...
        id = ctx.author.id
        cur.execute(
            'SELECT osu_user_id, osu_username FROM user WHERE discord_user_id = ?',
            (id, ))
        ...
        recent = api.get(
            f"/users/{user_id}/scores/recent?include_fails=1&limit=50")
        ...
        username = jsonData[0]['user']['username']
        ...
        embed = await _recent(user_id, index)
    ...
    message_id = await ctx.send(
        "**Recent osu! Standard Play for {}:**".format(username),
        embed=embed,
        view=view)
    cuser = await bot.fetch_user(ctx.author.id)
    print("コマンドが使用されました：.rs {} [ {} ]".format(username, cuser))
    ...
    cur.execute(
        "INSERT INTO rs_temp(message_id, user_id, username, i, max) VALUES(?, ?, ?, ?, ?)",
        (
            message_id.id,
            user_id,
            username,
...
async def _recent(user_id, i):

    datetime_recent_ = []

    recent = api.get(
        f"/users/{user_id}/scores/recent?include_fails=1&limit={i + 1}")
    jsonData = recent.json()
    ...
    if pp_fc is None:
        embed = discord.Embed(
            description=
            "▸ Rank: **{}**\n▸ Performance: **{:.2f}PP**\n▸ Accuracy: **{:.2f}%**\n▸ Score: {:,d}\n▸ Combo: x{}/{} [{}/{}/{}/{}]"
            .format(rank, pp_curr, acc, score, combo, maxcombo, count_300,
                    count_100, count_50, count_miss),
            color=discord.Colour.green())
        embed.set_author(
            name='{} - {} [{}] + {} [{:.2f}★]'.format(artist, title, version,
                                                      mod, difficulty),
            url="https://osu.ppy.sh/beatmaps/{}".format(beatmap_id),
            icon_url=str(avatar))
        embed.set_thumbnail(
            url="https://b.ppy.sh/thumb/{}l.jpg".format(mapset_id))
        ...
        embed = discord.Embed(
            description=
            "▸ Rank: **{}** ({:.1%})\n▸ Performance: **{:.2f}PP** ({:.2f}PP for {:.2%} FC)\n▸ Accuracy: **{:.2f}%**\n▸ Score: {:,d}\n▸ Combo: x{}/{} [{}/{}/{}/{}]"
            .format(rank, map_completion, pp_curr, pp_fc, acc_fc, acc, score,
                    combo, maxcombo, count_300, count_100, count_50,
                    count_miss),
            color=discord.Colour.green())
        embed.set_author(
            name='{} - {} [{}] + {} [{:.2f}★]'.format(artist, title, version,
...
async def osutop(ctx, *arg):
    ...
    if not arg:
        ...
        try:
            ...
            embed = discord.Embed(title="Error",
                                  description="**プレイヤーが見つかりませんでした。**")

            return await ctx.send(embed=embed)

    res = api.get(f"/users/{user_id}/scores/best?mode=osu&limit=100")
    jsonData = res.json()
    max = len(jsonData) // 5
    if len(jsonData) % 5 != 0:
        max += 1

    embed = await _osu_top(user_id, 1)
    ...
    cur.execute(
        "INSERT INTO osutop_temp(message_id, user_id, username, i, max) VALUES(?, ?, ?, ?, ?)",
        (
            message_id.id,
            user_id,
            username,
    ...
    while inter == True:
        cur.execute(
            "UPDATE osutop_temp SET inter = 'False' WHERE message_id = ?",
            (str(message_id.id), ))
        conn.commit()
        await asyncio.sleep(30)
...
async def _osu_top(user_id, page):
    ...
    try:
        game_id = api.get_username(user_id)
        res = api.get(
            f"/users/{user_id}/scores/best?mode=osu&limit={page * 5}")
        profile = api.get(f"/users/{user_id}/osu")
    except:
        embed = discord.Embed(title="Error",
                              description="**プレイヤーが見つかりませんでした。**")
    ...
    for i in range((page - 1) * 5, len(jsonData)):
        ...
        objects = jsonmap['count_circles'] + jsonmap[
            'count_sliders'] + jsonmap['count_spinners']
        ...
        pp = await pp_calc_user(jsonData[i]['beatmap']['id'],
                                jsonData[i]['rank'], jsonData[i]['mods'], acc,
                                combo, maxcombo,
                                jsonData[i]['statistics']['count_300'],
                                jsonData[i]['statistics']['count_100'],
                                jsonData[i]['statistics']['count_50'],
                                jsonData[i]['statistics']['count_miss'],
                                objects)
        ...
        if pp['pp_fc'] is None:
            top += "**{}. [{} - {} [{}]]({}) + {}** [{:.2f}★]\n▸ Rank: **{}** \n▸ Performance: **{:.2f}PP**\n▸ Accuracy: **{:.2f}%**\n▸ Score: {:,d}\n▸ Combo: x{}/{} [{}/{}/{}/{}]\n▸ Score Set: <t:{}:R>\n\n".format(
                ...
                jsonData[i]['beatmapset']['title'],
                jsonData[i]['beatmap']['version'],
            ...
            top += "**{}. [{} - {} [{}]]({}) + {}** [{:.2f}★]\n▸ Rank: **{}** \n▸ Performance: **{:.2f}PP** ({:.2f}PP for {:.2%} FC)\n▸ Accuracy: **{:.2f}%**\n▸ Score: {:,d}\n▸ Combo: x{}/{} [{}/{}/{}/{}]\n▸ Score Set: <t:{}:R>\n\n".format(
                ...
                jsonData[i]['beatmap']['version'],
                jsonData[i]['beatmap']['url'], mods, diff, jsonData[i]['rank'],
                jsonData[i]['pp'], pp_fc, acc_fc, acc, jsonData[i]['score'],
                combo, maxcombo, jsonData[i]['statistics']['count_300'],
                jsonData[i]['statistics']['count_100'],
                jsonData[i]['statistics']['count_50'],
                jsonData[i]['statistics']['count_miss'], int(datetime_recent))
    ...
    embed.set_author(name="Top osu! Standard Profile for {}".format(game_id),
                     url="https://osu.ppy.sh/users/{}".format(user_id),
                     icon_url=flag_url)
    embed.set_thumbnail(url=avatar)
...
async def osutop_top(inter: discord.Interaction):
    cur.execute("SELECT user_id, i, max FROM osutop_temp WHERE message_id = ?",
                (inter.message.id, ))
...
async def command(ctx):
    ...
    embed.add_field(name=".osuset ユーザー名", value="アカウントを紐付けする\n")

    embed.add_field(name=".osu ユーザー名(任意)", value="プロフィールを確認する\n", inline=False)

    embed.add_field(name=".osutop ユーザー名(任意)",
                    value="ベストパフォーマンスを確認する",
                    inline=False)

    embed.add_field(name=".rs ユーザー名(任意)", value="最新のプレイを確認する\n", inline=False)
    ...
    embed.add_field(
        name=".nbon",
        value='山田勝己が毎日1時に新着Ranked譜面のリストを送る\n **▸ ".nboff"** により登録を解除できます。',
        inline=False)
...
async def r(ctx, *args):
    ...
    temp = await _random(diff_min, diff_max, length)
    try:
        embed = temp[0]
        option = temp[1]
    except:
    ...
    print("コマンドが使用されました：.r {} [ {} ]".format(option, user))

    cur.execute(
        "INSERT INTO r_temp(message_id, diff_min, diff_max, length) values(?, ?, ?, ?)",
        (
            message_id.id,
            diff_min,
            diff_max,
    ...
    conn.commit()
    user = await bot.fetch_user(ctx.author.id)
    print("ボタンを削除しました：.r {} [ {} ]".format(option, user))


async def _random(diff_min, diff_max, length):
    ...
    for r in cur:
        pass
    if r != 0:
        embed = await beatmap_info(r[0], mods)
...
async def r_retry(inter: discord.Interaction):
    ...
    cur.execute(
        "SELECT diff_min, diff_max, length FROM r_temp WHERE message_id = ?",
        (inter.message.id, ))
    ...
    print("ボタンが押されました：.r {}".format(option))
...
async def pp(ctx, url, *mods):
    ...
    if jsonData['mode'] != 'osu':
        return await ctx.send("**osu!stdの譜面のみ計算できます。**")

    beatmap_id = jsonData['id']

    embed = await beatmap_info(beatmap_id, mods)
...
async def beatmap_info(beatmap_id, mods):
    ...
    pp = await pp_calc_map(map_id, mod_integer)
    ...
    embed = discord.Embed(
        title="{} - {} [{}]".format(artist, title, version),
        description=
        '▸ **Difficulty:** {:.2f}★\n▸ **Length:** {}:{}\n▸ **BPM:** {}\n▸ **CS:** {:.1f} **AR:** {:.1f} **OD:** {:.1f} **HP:** {:.1f}\n▸ **95%:** {:.2f}PP | **98%:** {:.2f}PP | **99%:** {:.2f}PP | **100%:** {:.2f}PP\n▸ **Status:** {}\n▸ [Download](https://osu.ppy.sh/beatmapsets/{}/download) | osu!direct: <osu://b/{}>'
        .format(diff_rating, y, x_zero, bpm, cs, ar, od, hp, pp_95, pp_98,
                pp_99, pp_ss, str.upper(status), mapset_id, map_id),
        url=map_url,
        color=discord.Colour.green())
...
async def pp_calc_user(map_id, rank, mod, acc, combo, maxcombo, count300,
                       count100, count50, misses, objects):
    ...
    calc = Calculator(mods=mod_integer)
    ...
    if maxcombo - combo <= count100 and misses == 0:
        ...
        count300 = objects - count100 - count50
        misses = 0
        ...
        calc.set_n_misses(misses)
        calc.set_combo(maxcombo)
        calc.set_passed_objects(objects)
        fc_perf = calc.performance(map)
        pp_json = {
            'pp_curr': curr_perf.pp,
            'pp_fc': fc_perf.pp,
...
async def pp_calc_map(map_id, mod):
    url = 'https://osu.ppy.sh/osu/{}'.format(map_id)
...
time = datetime.time(hour=16, minute=0, second=0, tzinfo=utc)
    ...
    while (flag == 0):
        ...
        if len(req_json) == 0:
            ...
            for i in range(len(Channel_id)):
                ...
                try:
                    ...
                    cur.execute(
                        "DELETE FROM notice_new_beatmaps WHERE channel_id = ?",
                        (Channel_id[i], ))
                    conn.commit()
        ...
        for i in range(len(req_json)):
            ...
            if req_json[i]["approved"] == "1" or req_json[i]["approved"] == "2":
                ...
                beatmapset_Max_difficulty.append(
                    req_json[i]["difficultyrating"])
                beatmapset_Min_difficulty.append(
                    req_json[i]["difficultyrating"])
                ...
                for j in range(len(beatmapset_id) - 1):
                    if req_json[i]["beatmapset_id"] == beatmapset_id[j]:
                        if req_json[i][
                                "difficultyrating"] < beatmapset_Max_difficulty[
                                    j]:
                            beatmapset_Max_difficulty[
                                len(beatmapset_Max_difficulty) -
                                1] = beatmapset_Max_difficulty[j]
                        if req_json[i][
                                "difficultyrating"] > beatmapset_Min_difficulty[
                                    j]:
                            beatmapset_Min_difficulty[
                                len(beatmapset_Min_difficulty) -
            ...
            if temp != -1:
                try:
                    ...
                    if jsonData['mode'] == 'osu' and (jsonData['ranked'] == 1
                        ...
                        total_length = float(jsonData['total_length']) / 60
                        diff_min = round(float(beatmapset_Min_difficulty[i]),
                                         2)
                        diff_max = round(float(beatmapset_Max_difficulty[i]),
                                         2)
        ...
        for i in range(len(maps)):
            if i % 15 == 0 and i != 0 and i != len(maps) - 1:
                embed = discord.Embed(title="新着譜面一覧",
                                      description=mapstr,
                                      color=discord.Colour.green())
                embed.set_thumbnail(url="https://b.ppy.sh/thumb/{}l.jpg".
                                    format(beatmapset_id[0]))
        ...
        print(
            f'次回は、{approved_date[len(approved_date) - 1]}以降にランク付けされた譜面を出力します。\n'
        )
...
async def add_beatmap():
    ...
    while (flag == 0):
            ...
            if req_json[i]["approved"] != '3':
                ...
                if temp != -1:
                    try:
                        ...
                        if jsonData['mode'] == 'osu' and (
                                jsonData['ranked'] == 1
                                or jsonData['ranked'] == 2):

                            map = Beatmap(path=file_path)

                            calc = Calculator(mods=0)

                            perf_max = calc.performance(map)
                            calc.set_difficulty(perf_max.difficulty)

                            pp_aim = perf_max.pp_aim
                            pp_acc = perf_max.pp_acc
                            pp_speed = perf_max.pp_speed

                            cur.execute(
                                'INSERT INTO beatmaps(beatmap_id, artist, title, version, type_id, difficulty, bpm, length, pp_aim, pp_tap, pp_acc) values(?, ?, ?, ?, (SELECT id FROM maptype WHERE type = ?), ?, ?, ?, ?, ?, ?) ON CONFLICT(beatmap_id) DO UPDATE SET type_id = (SELECT id FROM maptype WHERE type = ?), artist = ?, title = ?, version = ?, bpm = ?, difficulty = ?, length = ?, pp_aim = ?, pp_tap = ?, pp_acc = ?',
                                (
                                    beatmap_id[i],
                            ...
                            conn.commit()
                            print("Added Beatmap to DB：{}".format(
                                beatmap_id[i]))
                            count += 1
                        else:
                            print("Beatmap is not Ranked：{}".format(
                                beatmap_id[i]))
                    except:
                        print("Not Found：{}".format(beatmap_id[i]))
                    os.remove(file_path)
        ...
        print(
            f'次回は、{approved_date[len(approved_date) - 1]}以降にランク付けされた譜面を登録します。\n'
        )
